...
# 切分出来的前30s由于不一定是正好在语气停顿点上，会导致模型幻觉问题
...
def train_voice():
    ...
    if audio_file.filename.endswith('.mp3'):
        # 定义 MP3 文件的保存路径
        print("上传的是mp3文件")
        mp3_path = os.path.join('uploads', f'{voiceid}.mp3')
        audio_file.save(mp3_path)
        audio = AudioSegment.from_mp3(mp3_path)
        # 如果音频时长大于30s，则截取前28s
        if len(audio) > 30 * 1000:
            print("mp3音频时长大于30s，截取前28s")
            audio = audio[:28 * 1000]
...
if __name__ == '__main__':
    app.run(port=7993, host='0.0.0.0', use_reloader=False)
